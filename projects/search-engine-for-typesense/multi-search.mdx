...
Related: <a href="/projects/search-engine-for-typesense/client">Client</a>, <a href="/projects/search-engine-for-typesense/multi-search-guide">Multi-search Guide</a>
...
res = SearchEngine.multi_search(common: { query_by: SearchEngine.config.default_query_by }) do |m|
  ...
  m.add :brands,   Publisher.where('name:~rud').per(5)
end

res[:books].found
res.dig(:brands).to_a
res.labels #=> [:books, :brands]
```
...
res = SearchEngine.multi_search(common: { q: 'milk', per_page: 50 }) do |m|
  ...
  m.add :brands,   Publisher.all           # per_page not present, inherits 50
end
```
...
```ruby
mr = SearchEngine.multi_search_result { |m| m.add :books, rel1; m.add :brands, rel2 }
mr[:books].found
mr.dig(:brands).to_a
mr.labels # => [:books, :brands]
```
...
- <a href="/projects/search-engine-for-typesense/client">Client</a> for URL/common params and error mapping
- <a href="/projects/search-engine-for-typesense/relation-reference">Relation Guide</a> for query composition and compilation
- <a href="/projects/search-engine-for-typesense/configuration">Configuration</a> for cache knobs and <code>multi_search_limit</code>

---

## Observability

<a href="/projects/search-engine-for-typesense/observability">Observability</a> · <a href="/projects/search-engine-for-typesense/dx">DX</a> · <a href="/projects/search-engine-for-typesense/testing">Testing</a>
