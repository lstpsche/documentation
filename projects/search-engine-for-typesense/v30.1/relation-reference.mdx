...
| `vector_search(field, k:, alpha:, ...)` | Semantic / hybrid / ANN search on an embedding field. Last call wins. | `vector_query` |
| `find_similar(id, field:, k:, ...)` | Find documents similar to a given document ID (sugar over `vector_search` with `id:`) | `vector_query` |
