...
- Registers callbacks on your AR model (installed once per class reload):
  - <code>after_create_commit</code> → upsert document (post-transaction)
  - <code>after_update_commit</code> → upsert document (post-transaction)
  - <code>after_destroy_commit</code> → delete document (post-transaction)
