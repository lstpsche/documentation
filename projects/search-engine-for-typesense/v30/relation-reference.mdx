...
Related: <a href="/projects/search-engine-for-typesense/v30/relation">Relation</a>, <a href="/projects/search-engine-for-typesense/v30/query-dsl">Query DSL</a>, <a href="/projects/search-engine-for-typesense/v30/dx">DX</a>, <a href="/projects/search-engine-for-typesense/v30/debugging">Debugging</a>
...
See also: <a href="/projects/search-engine-for-typesense/v30/relation">Relation</a> and <a href="/projects/search-engine-for-typesense/v30/query-dsl">Query DSL</a>.
...
- OR semantics require a raw fragment (e.g., "a:=1 || b:=2") or higher‑level AST usage (see <a href="/projects/search-engine-for-typesense/v30/query-dsl">Query DSL</a>)
...
See: <a href="/projects/search-engine-for-typesense/v30/dx">DX</a> to preview compiled params. NOT-IN is rendered as <code>NOT IN [...]</code> in explain output.
...
See: <a href="/projects/search-engine-for-typesense/v30/grouping">Grouping</a>, <a href="/projects/search-engine-for-typesense/v30/relation-reference">Relation Guide</a>, <a href="/projects/search-engine-for-typesense/v30/field-selection">Field Selection</a>.
...
| `vector_search(field = nil, k:, alpha:, ...)` | Semantic / hybrid / ANN search on an embedding field. `field` auto-resolved when the model has one embedding. Last call wins. | `vector_query` |
| `find_similar(id, field: nil, k:, ...)` | Find documents similar to a given document ID (sugar over `vector_search` with `id:`). `field:` auto-resolved when the model has one embedding. | `vector_query` |
| `where_geo(field, within_radius: / within_polygon:)` | Filter by geographic radius or polygon containment. Field must be `:geopoint`. | `filter_by` |
| `order_geo(field, from:, direction:, exclude_radius:, precision:)` | Sort by distance from a reference point. Optional `exclude_radius` and `precision`. | `sort_by` |
| `order_eval(expression, direction:)` | Sort by `_eval()` conditional expression. Accepts a string or weighted `[{ expr:, weight: }]` array. | `sort_by` |

See <a href="/projects/search-engine-for-typesense/v30/vector-search">Vector Search</a> for configuration, `embedding` macro, and query examples.
See <a href="/projects/search-engine-for-typesense/v30/geo-search">Geo Search</a> for geo filtering, distance sorting, and `_eval()` viewport boost patterns.
...
- <strong>Joins</strong>: Declare associations on the model, apply with <code>.joins(:assoc)</code>, then filter
  ...
  <a href="/projects/search-engine-for-typesense/v30/joins">Joins</a> and
  <a href="/projects/search-engine-for-typesense/v30/field-selection">Field Selection</a>.
- <strong>Presets</strong>: Attach server‑side bundles of defaults and choose a mode
  ...
  <a href="/projects/search-engine-for-typesense/v30/presets">Presets</a> and
...
See: <a href="/projects/search-engine-for-typesense/v30/dx">DX</a>, <a href="/projects/search-engine-for-typesense/v30/observability">Observability</a>.
...
See: <a href="/projects/search-engine-for-typesense/v30/compiler">Compiler</a> for precedence, quoting rules, and join context.
...
- <strong>Quoting</strong>: strings are double‑quoted; booleans <code>true/false</code>; <code>nil</code> → <code>null</code>; arrays are one‑level
  ...
  numeric <code>:time/:datetime</code> fields prefer epoch seconds; <code>:time_string/:datetime_string</code> use ISO8601 (see <a href="/projects/search-engine-for-typesense/v30/compiler">Compiler</a>).
- <strong>Booleans vs strings</strong>: boolean fields coerce "true"/"false"; other fields treat strings literally (see <a href="/projects/search-engine-for-typesense/v30/joins">Joins</a>).
...
- <strong>Joined fields</strong>: require <code>.joins(:assoc)</code> before filtering/sorting/selection on <code>$assoc.field</code> (see <a href="/projects/search-engine-for-typesense/v30/joins">Joins</a>).
- <strong>Grouping field</strong>: base fields only; joined paths like <code>$assoc.field</code> are rejected (see <a href="/projects/search-engine-for-typesense/v30/grouping">Grouping</a>).
...
See <a href="/projects/search-engine-for-typesense/v30/faceting">Faceting</a> for first-class faceting DSL: <code>facet_by</code>, <code>facet_query</code>, compiler mapping and result helpers.

---

Related links: <a href="/projects/search-engine-for-typesense/v30/relation">Relation</a>, <a href="/projects/search-engine-for-typesense/v30/query-dsl">Query DSL</a>, <a href="/projects/search-engine-for-typesense/v30/dx">DX</a>,
<a href="/projects/search-engine-for-typesense/v30/joins">Joins</a>, <a href="/projects/search-engine-for-typesense/v30/field-selection">Field Selection</a>, <a href="/projects/search-engine-for-typesense/v30/compiler">Compiler</a>,
<a href="/projects/search-engine-for-typesense/v30/grouping">Grouping</a>, <a href="/projects/search-engine-for-typesense/v30/observability">Observability</a>
