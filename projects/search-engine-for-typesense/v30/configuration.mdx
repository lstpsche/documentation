...
Related: <a href="/projects/search-engine-for-typesense/v30/installation">Installation</a>, <a href="/projects/search-engine-for-typesense/v30/client">Client</a>, <a href="/projects/search-engine-for-typesense/v30/cli">CLI</a>
...
| `strict_fields`    | Boolean              | `true` in development/test; else `false` | Parser validates unknown fields when `true`; see <a href="/projects/search-engine-for-typesense/v30/query-dsl#error-reference">Query DSL</a> |
...
| `syncable_callback_timing` | Symbol   | `:after_commit` | Controls ActiveRecordSyncable callback timing. `:after_commit` uses post-transaction callbacks; `:after_save` uses legacy in-transaction callbacks |
...
- Defaults to `true` in Rails test (`Rails.env.test?`) or when `RACK_ENV=test`
  is set
...
- If `SearchEngine.config.client` is set (e.g., `StubClient`), it always takes
  precedence
...
See <a href="/projects/search-engine-for-typesense/v30/observability">Observability</a> for event payloads and redaction.
...
Settings for vector search auto-embedding. See <a href="/projects/search-engine-for-typesense/v30/vector-search">Vector Search</a> for the full guide.

```ruby
SearchEngine.configure do |c|
  c.embedding.model = 'ts/all-MiniLM-L12-v2'
  c.embedding.api_key = ENV['OPENAI_API_KEY'] # only for remote providers
  c.embedding.model_config = {}               # extra keys merged into embed block
end
```

| Field | Default | Notes |
| --- | --- | --- |
| `model` | `nil` | Typesense model name (e.g. `ts/all-MiniLM-L12-v2`, `openai/text-embedding-3-small`) |
| `api_key` | `nil` | API key for remote embedding providers |
| `model_config` | `nil` | Extra keys merged into the Typesense `embed.model_config` block |
