...
- <code>SearchEngine::Schema.apply!(klass)</code> → create new physical, reindex, atomically swap alias, enforce retention; returns `{ logical, new_physical, previous_physical, alias_target, dropped_physicals }`.
...
- Always present on every collection.
