---
title: "Reindexing"
description: "Fastest ways to rebuild with schema changes."
---

## Related links
- <a href="/projects/search-engine-for-typesense/schema">Schema</a>
- <a href="/projects/search-engine-for-typesense/cli">CLI</a>
- <a href="/projects/search-engine-for-typesense/observability">Observability</a>
- <a href="/projects/search-engine-for-typesense/troubleshooting">Troubleshooting</a>

Use this page when you need a quick, safe reindex.

<Note>
  Examples assume the <code>SearchEngine::Product</code> model from
  <a href="/projects/search-engine-for-typesense/guidebook-model-configuration">
    Model configuration
  </a>.
</Note>

## Fastest ways to reindex

<Steps>
<Step title="CLI (safe blue/green)">
  ```bash
  rails 'search_engine:schema:apply[products]'
  rails 'search_engine:schema:rollback[products]'
  ```
</Step>

<Step title="Ruby (auto fix drift)">
  ```ruby
  SearchEngine::Product.index_collection(pre: :index)
  SearchEngine::Bulk.index_collections(:products, :brands)
  ```
</Step>
</Steps>

<Warning>
  <code>reindex_collection!</code> and <code>Bulk.reindex_all!</code> drop the
  active physical collection. Use only when destructive reindexing is safe.
</Warning>

## Destructive reindex (only if safe)

```ruby
SearchEngine::Product.reindex_collection!
SearchEngine::Bulk.reindex_all!
```

## Retention settings

Keep a limited number of old physical collections after swaps:

```ruby
class SearchEngine::Product < SearchEngine::Base
  schema_retention keep_last: 2
end
```

## Next steps

- See <a href="/projects/search-engine-for-typesense/schema">Schema</a> for
  diff rules and retention behavior.
- Use <a href="/projects/search-engine-for-typesense/observability">Observability</a>
  to track schema apply and reindex events.
