...
Related: <a href="/projects/search-engine-for-typesense/v29/cli">CLI</a>, <a href="/projects/search-engine-for-typesense/v29/troubleshooting#schema">Troubleshooting → Schema</a>
...
- <code>SearchEngine::Schema.prune_orphans!(client: nil, logical: nil)</code> → drop orphaned physical collections that no alias points to. Scoped to a single logical name when `logical:` is provided; scans all collections when `nil`. Returns `{ dropped: Array, kept: Array, total_scanned: Integer }`.
...
- Reindexing is required. Provide a block to <code>apply!</code> or implement <code>klass.reindex_all_to(physical_name)</code> to perform bulk import. On failure or interruption, no alias swap occurs and the new physical collection is automatically deleted to prevent orphan accumulation.
...
When <code>Schema.apply!</code> is used with the high-level <code>.index_collection</code> flow, the engine monitors indexation results. If the result status is not <code>:ok</code> (e.g., <code>:partial</code> or <code>:failed</code>), the block raises <code>SearchEngine::Errors::IndexationAborted</code>, which prevents the alias swap. The newly created physical collection is automatically cleaned up by the <code>ensure</code> block in <code>apply!</code>, preventing orphan accumulation. The same cleanup applies when indexing is interrupted (e.g., via <code>Ctrl+C</code>).
...
Physical collections can accumulate when blue/green deployments fail before the alias swap occurs. <code>Schema.prune_orphans!</code> scans all collections matching the timestamped naming pattern (<code>logical_YYYYMMDD_HHMMSS_###</code>) and drops any that are not the target of an alias.
...
A rake task is also available (see <a href="/projects/search-engine-for-typesense/v29/cli#commands">CLI</a>):
...
See also: <a href="/projects/search-engine-for-typesense/v29/client">Client</a>, <a href="/projects/search-engine-for-typesense/v29/configuration">Configuration</a>, and <a href="/projects/search-engine-for-typesense/v29/compiler">Compiler</a>.
...
- <strong>Orphaned physicals accumulating</strong>: Run <code>Schema.prune_orphans!</code> or the <code>schema:prune_orphans</code> task to clean up physicals left by failed deployments.
- <strong>Alias not swapped after apply</strong>: If indexation returned a non-ok status, <code>IndexationAborted</code> prevented the swap. Inspect the new physical and retry, or fix import errors first.

Backlinks: <a href="https://github.com/lstpsche/search-engine-for-typesense#readme" target="_blank">README</a>, <a href="/projects/search-engine-for-typesense/v29/indexer">Indexer</a>
