...
When indexation is run inside <code>Schema.apply!</code> (e.g., via <code>.index_collection</code>), a non-ok result raises <code>SearchEngine::Errors::IndexationAborted</code>, which prevents the alias swap. The partially-indexed new physical is left intact for inspection. See <a href="/projects/search-engine-for-typesense/v29/schema#failed-indexation-safety">Schema → Failed indexation safety</a>.
