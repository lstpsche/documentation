...
```ruby
SearchEngine::Product.where(brand_id: "brand_1", tags: ["sale", "new"])
SearchEngine::Product.where(["price >= ?", 100])
SearchEngine::Product.where("price:>=100 && tags:=[\"sale\"]")
SearchEngine::Product.where.not(status: "archived")
```
...
```ruby
SearchEngine::Product.page(2).per(25)
SearchEngine::Product.limit(50) # fallback when page/per are not set
```
